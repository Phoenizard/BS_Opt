...
if __name__ == '__main__':
    ...
    # 【已修正】调用时传入现货价格 ST
    C_pred, C_pred_opt_1, C_pred_opt_2, loss_opt, mu_opt, pi_opt = simulation_LOOCV(0.071, C_obs, n, X, ST, r, tau, d)

    #计算误差百分比
    loss = np.mean(np.abs(C_obs - C_pred) / C_obs)
    loss_opt_percent = np.mean(np.abs(C_obs - C_pred_opt_2) / C_obs)
    print('Mean loss:', loss) 
    print('Mean loss optimized (percentage):', loss_opt_percent)

    print('Optimized mu:', mu_opt)
    print('Optimized pi:', pi_opt)
    
    sigma = 0.071
    # 注意：这里的 calculate_and_plot_expression 是为单一执行价设计的，但为了演示，我们传入X数组
    # 该函数内部会使用X的均值进行绘图
    calculate_and_plot_expression(pi_opt, mu_opt, sigma, X, r, tau, 1.5*ST)

    plt.plot(X, C_obs, 'o', label='Observed')
    plt.plot(X, C_pred_opt_2, 'r-', label='Optimized Fit')
    plt.legend()
    plt.show()